...
(stored-distance) limit these implementations to a tens of thousands of
...
motivated this work, the distance matrix is already computed in a previous
...
<<linkages>>=
Rclusterpp.linkage_kinds()
@
and the following distance metrics:
<<distances>>=
Rclusterpp.distance_kinds()
@
...
Table~\ref{tab:complexity} shows the worst-case time and space
...
and space using RNN and SLINK~\cite{Sibson1973} algorithms
respectively. While average and complete-link trade increased time bounds,
$O(n^3)$ versus $O(n^2)$, in exchange for reducing the memory footprint to
$O(n)$ from $O(n^2)$.
...
server. \code{RclusterppDistance} is the stored-distance method and \code{Rclusterpp} is the stored data implementation.}
\begin{tabular}{l c c}\small
Implementation & Exec. Time (s) & $n$ \\
\hline
